...
import { createAnimationWithSlides, updateAnimationWithSlides } from "@/lib/supabase/queries";
...
export function useSaveAnimation() {
    ...
    const saveAnimation = async (name: string, description: string, forceCreate = false) => {
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            if (!forceCreate && animationId) {
                const updateError = await updateAnimationWithSlides(
                    supabase, animationId, name, description, user, slides
                );
                ...
                const {data, error:createError} = await createAnimationWithSlides(
                    supabase, name, description, user, slides
                );
                if (createError) throw createError;
                if (!forceCreate) {
                    setAnimationDetails(data.id, name, description);
                }
                toast({
                    title: "Success",
            ...
            const errorMessage = err instanceof Error ? err.message : 'Failed to save animation';
