...
const GradientPicker = forwardRef<
    HTMLInputElement,
    Omit<React.ComponentProps<typeof Button>, 'value' | 'onChange' | 'onBlur'> & GradientPickerProps
>(
    (
        { disabled, value, onChange, onBlur, name, className, direction = 'to right', ...props },
        forwardedRef
    ) => {
        ...
        const handlePositionChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
        ...
        const addStop = () => {
            if (gradientStops.length < 5) {
                ...
                onChange([...gradientStops, { color: newColor, position: newPosition }]);
